import{i as l,c as b,K as P,u as x,l as ae,s as v}from"./assets/footer-8dde7164.js";import{a as q,P as re,N as ie,l as ne}from"./assets/vendor-4b895a38.js";const ce="https://food-boutique.b.goit.study/api/",le="products/categories",de="products",ue="products/",pe="products/popular",fe="products/discount",ge="subscription",ye="orders",h=q.create({baseURL:ce});class me{static async getCategories(){return(await h.get(le)).data}static async getProducts({keyword:e,category:s,page:o,limit:r,sortBy:a}={}){return(await h.get(de,{params:{keyword:e,category:s,page:o,limit:r,[a==null?void 0:a.key]:a==null?void 0:a.value}})).data}static async getProduct(e){return(await h.get(ue+e)).data}static async getPopularProducts(e=5){return(await h.get(pe,{params:{limit:e}})).data}static async getDiscountedProducts(){return(await h.get(fe)).data}static async subscribe(e){return(await h.post(ge,{email:e})).data}static async placeOrder(e){const{email:s,products:o}=e;return(await h.post(ye,{email:s,products:o.map(({productId:a,amount:i})=>({productId:a,amount:i}))})).data}}const A=document.querySelector(".modal-background"),j=document.querySelector(".modal");async function M(t){try{A.classList.remove("is-hidden"),document.body.classList.add("is-modal-open"),j.innerHTML=`
    ...
    `;const e=await me.getProduct(t);j.insertAdjacentHTML("beforeend",be(e)),document.querySelector(".modal-btn").addEventListener("click",()=>{ve(e)}),document.querySelector(".modal-btn-remove").addEventListener("click",()=>{he(e)});for(let s=0;s<b.length;s++)b[s]._id===t&&(document.querySelector(".modal-btn").style.display="none",document.querySelector(".modal-btn-remove").style.display="block");document.querySelector(".modal-close-btn").addEventListener("click",C),A.addEventListener("click",Y),document.addEventListener("keydown",K)}catch(e){console.error("Error fetching product data:",e.message)}finally{}}function be({img:t,name:e,category:s,size:o,popularity:r,desc:a,price:i}){return`
      ...
      <svg class="modal-icon-shop" width="18" height="18">
        <use href="${l}#icon-shopping-cart"></use>
                ...
                <p class="not-found-message">Try adjusting your search parameters or browse our range by other criteria
                    ...
                    product for
                    you.</p>
            </div>`,o=t.map(({img:r,name:a,popularity:i,category:n,price:u,size:y,_id:d,is10PercentOff:E})=>{const N=n.replace(/_/g," "),k=e?JSON.parse(e).some(se=>se._id===d):!1,te=E?"icon-discount-pl":"visually-hidden",oe=k?`${l}#icon-cart`:`${l}#icon-shopping-cart`;return`<li class="item-pl" data-id="${d}">
